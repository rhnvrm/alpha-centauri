...
export function createGame(missionId = 'firstLight', sessionId = id('session')) {
  ...
  return {
    ...
    observedWorld: { buildings: scenario.buildings.map(({ id, type, x, y }) => ({ id, type, x, y, status: 'complete' })), robots: structuredClone(scenario.robots).map(({ id, type, x, y, status, lifecycle, purpose, workstream, path }) => ({ id, type, x, y, status, lifecycle: lifecycle || status, purpose: purpose || null, workstream: workstream || null, routeRemaining: (path || []).length })), roads: seededRoads(scenario).map((r) => ({ x: r.x, y: r.y })) },
...
export function telemetryFor(state) {
  return {
    ...
    observedWorld: {
      ...
      robots: state.robots.map(({ id, type, x, y, status, lifecycle, assignedJob, path, purpose, workstream }) => ({
        id, type, x, y, status, lifecycle: lifecycle || status, assignedJob: assignedJob || null,
        routeRemaining: (path || []).length, purpose: purpose || null, workstream: workstream || null,
      })),
